...
const MainLayout = ({ onBack }) => {
  ...
  // A REF que aponta para o currículo
  const componentRef = useRef();

  // --- CONFIGURAÇÃO DE IMPRESSÃO CORRIGIDA (VISIBILIDADE SELETIVA) ---
  const handlePrint = useReactToPrint({
    ...
    pageStyle: `
      @page {
        size: A4;
        margin: 0mm; /* Margens geridas pelo template */
      }
      @media print {
        ...
        /* 2. Torna visível APENAS o contentor do currículo e os seus filhos */
        .print-container, .print-container * {
          visibility: visible !important;
        }

        /* 3. Posiciona o currículo no topo esquerdo da folha, ignorando o layout do site */
        .print-container {
          ...
          transform: none !important; /* Remove a escala do ecrã */
        }

        /* 4. Garante que imagens e cores de fundo são impressas */
        * {
    ...
    <div className="flex h-screen flex-col md:flex-row overflow-hidden bg-background-light dark:bg-background-dark text-gray-900 dark:text-gray-100 transition-colors duration-300 print:hidden">
      ...
      <main className="flex-1 h-1/2 md:h-full bg-gray-100 dark:bg-[#1e1e1e] p-4 md:p-8 flex justify-center items-start overflow-y-auto relative">
        ...
        <div className="bg-white text-black shadow-material-3 w-[210mm] h-auto min-h-[297mm] origin-top transform scale-50 md:scale-75 lg:scale-[0.65] xl:scale-75 2xl:scale-90 transition-transform duration-300 p-0">
          ...
          {/* IMPORTANTE: O ref está AQUI. 
              Adicionei a classe 'print-container' para o CSS de impressão a encontrar.
          */}
